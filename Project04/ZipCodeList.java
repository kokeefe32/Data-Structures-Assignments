...
/**
 * ZipCodeList contains collision objects that all occurred in the same zip
...
public class ZipCodeList  implements Iterable <Collision>, Comparable<ZipCodeList>{
	...
	// Implement LinkedList rather than ArrayList (which was in the original code)
	private LinkedList<Collision> listLinked = new LinkedList<Collision>(); 
	...
	public ZipCodeList ( Collision col ) { 
		listLinked = new LinkedList<Collision> ();
		zip = col.getZip();
		add(col);
	}
